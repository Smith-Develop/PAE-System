...
export async function GET() {
  ...
  return NextResponse.json(await prisma.purchase.findMany({ where, include: { product: { include: { masterProduct: true, provider: true } }, operator: true, client: true }, orderBy: { fechaCompra: "desc" } }));
}
...
export async function POST(request: Request) {
  try {
    ...
    const purchase = await prisma.purchase.create({ data: { ...result.data, tenantId: tenantId || undefined }, include: { product: { include: { masterProduct: true } } } });
