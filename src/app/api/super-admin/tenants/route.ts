...
export async function POST(request: Request) {
  ...
  try {
    const tenant = await prisma.tenant.create({ data: { name, slug: slug.toLowerCase().replace(/\s+/g, "-"), planId: planId || null, active: active ?? true, maxUsers: maxUsers ?? 5, aiScansLimit: aiScansLimit ?? 10 } });
