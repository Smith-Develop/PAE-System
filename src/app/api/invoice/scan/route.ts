...
// Convierte un PDF (base64) a imagen JPEG (base64) usando sharp/libvips
async function pdfToImage(pdfBase64: string): Promise<{ base64: string; mimeType: string }> {
  const pdfBuffer = Buffer.from(pdfBase64, "base64");
  const sharp = (await import("sharp")).default;
  const jpegBuffer = await sharp(pdfBuffer, { pages: 1, page: 0 }).jpeg({ quality: 90 }).toBuffer();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    let rawBase64 = image.replace(/^data:image\/\w+;base64,/, "").replace(/^data:application\/pdf;base64,/, "");
    let mimeType: string;

    // Convertir PDF a imagen si es necesario (para OpenAI, DeepSeek, etc.)
    if (isPdf && aiModel.provider !== "google") {
      try {
        ...
        return NextResponse.json({
          error: "No se pudo convertir el PDF a imagen. Sube una imagen JPG/PNG de la factura, o usa un modelo de Google para PDFs.",
        }, { status: 400 });
      }
    } else {
    ...
    if (aiModel.provider === "google") {
      ...
      text = await scanWithOpenAI(aiModel.apiKey, aiModel.modelId, aiModel.baseUrl, rawBase64, mimeType);
    }
